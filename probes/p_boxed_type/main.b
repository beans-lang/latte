...
// one?
//
// It used to box the static type, so an object held through a base-class
// binding lost what it was and `as?` back to the subclass answered none —
// reflection could not recover a subclass from a base-class binding, which is
// most of what a framework wants it for. Fixed in 0.1.41 (#163). The Renderer
// holds its page as a `Component`, and the mount pass needs the concrete type
// to find its fields, so this is the exact question.
package main
