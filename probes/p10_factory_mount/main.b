...
// both backends, where reflective activation (B7) does not?
//
...
//   C3  reflective activation of a subclass of a closed generic — must FAIL
//       natively (B7). If C3 passes natively, this compiler is not the one B7
//       was recorded against and nothing below means what it says.
package main
...
pub class OrderGrid extends Grid<int> {             // C3: B7's shape
...
pub class Host {
    ...
    /// generic entry point is an INSTANCE method because a free function or a
    /// static taking a `fn(T)` type-checks, runs, and cannot be built (B3).
...
/// REFLECTIVELY. B1 says a reflective field access on a field declared by a
/// generic type is `ok` interpreted and `err unsupported` natively. If that is
/// still true, the factory route mounts a generic component fine and a `@param`
/// on one still diverges — so the scan has to refuse it by name.
...
fn main() {
    ...
    reflective("OrderGrid  (B7: generic base)      ", type_of(OrderGrid))
    reflective("Grid<int>  (B1: closed generic)    ", type_of(Grid<int>))
