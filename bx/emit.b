...
pub class Emitted {
    ...
    /// Top-level declarations the markup produced: one class per
    /// `<RenderBlock>`, whose body is a component of its own.
    pub blocks: List<string> = []
    pub diags: List<Diag> = []
...
pub class Emitter {
    ...
    loops: int = 0
    /// The class the generated `render` belongs to. A `<RenderBlock>` body
    /// becomes a class named after it, so the name has to be known here.
    pub owner: string = ""
    /// The classes those bodies produced, in source order.
    blocks: List<string> = []
    /// Whether this emitter is writing a `<RenderBlock>` body. Inside one,
    /// `self` is the parent's and does not exist in the runtime the block may
    /// run in, so naming it is refused where it is written.
    in_block: bool = false
    /// How many `<RenderBlock>`s this file has emitted, so each body's class
    /// has a name of its own. Counted rather than taken from the sequence
    /// number: two blocks in two arms of a `$if` draw disjoint numbers, but a
    /// block inside a `$for` body restarts at 0 and would collide.
    block_count: int = 0
    ...
    fn check_code(code: string, at: Span, what: string) {
        ...
        if self.in_block && uses_identifier(code, "self") {
            self.report(at, "{what} names `self` inside a <{RENDER_BLOCK_TAG}>, and that is the PARENT's self — a block runs in its own region, which may be a different runtime, so the parent's instance, its services and its handles are not there. Pass what you need as a prop and read it off `props`")
        }
    ...
    fn node(node: Node, indent: int) {
        ...
        match node as? ElementNode {
            some(element) => {
                if element.block { self.emit_render_block(element, indent) }
                else if element.component { self.emit_component(element, indent) }
    ...
    fn emit_component(element: ElementNode, indent: int) {
        ...
        let mode: string = self.mode_of(element)
        let seq: int = self.counters.next()
        self.setup_depth = self.setup_depth + 1
        let c: string = self.setup_name()
        // `component` and not `component_in("")` for a tag that named no
        // mode, so every file generated before render modes existed is still
        // generated byte for byte.
        if mode == "" {
            self.write(indent, "{b}.component<{element.tag}>({seq}, fn({c}: {element.tag}) \{{self.trace(element.span)}")
        } else {
            self.write(indent, "{b}.component_in<{element.tag}>({seq}, \"{escape_beans_string(mode)}\", fn({c}: {element.tag}) \{{self.trace(element.span)}")
        }
        for attr: Attr in element.attrs {
    ...
    /// The `render:mode=` or `mode=` this tag wrote, or `""`.
    ///
    /// A word the runtime does not know is refused HERE and not there: the
    /// runtime's refusal arrives while a page is rendering, and this one
    /// arrives with a file and a line.
    fn mode_of(element: ElementNode) -> string {
        for attr: Attr in element.attrs {
            match attr as? ModeAttr {
                some(mode) => {
                    if !mode_word_is_known(mode.value) {
                        self.report(mode.span, "{mode.value} is not a render mode — write static, server, client or auto")
                        return ""
                    }
                    if mode.value == "inherit" {
                        self.report(mode.span, "inherit is the absence of a mode — write no render:mode at all to inherit the region around this tag")
                        return ""
                    }
                    return mode.value
                }
    ...
    /// `<RenderBlock mode="client" count:int={self.n}> … </RenderBlock>`.
    ///
    /// The body becomes a component of its own, declared beside the one this
    /// file is named for. That is what an execution boundary needs and what a
    /// flag on an existing component could never be: its own instance, its
    /// own lifecycle, its own numbering, and — because it is a separate class
    /// with `@param` fields — a props contract beansc checks.
    ///
    /// Inside the body `props` is the block's own instance. `self` is
    /// refused: it would be the PARENT's, which does not exist in the runtime
    /// the block may be running in.
    fn emit_render_block(element: ElementNode, indent: int) {
        let mode: string = self.mode_of(element)
        if mode == "" {
            var wrote_one: bool = false
            for attr: Attr in element.attrs {
                match attr as? ModeAttr { some(_) => { wrote_one = true } none => {} }
            }
            if !wrote_one {
                self.report(element.span, "<{RENDER_BLOCK_TAG}> needs a mode: it exists to put its body in a different one from the region around it, so <{RENDER_BLOCK_TAG} mode=\"client\"> is the whole of what it is for")
            }
            return
        }
        if !self.type_params.is_empty() {
            self.report(element.span, "<{RENDER_BLOCK_TAG}> inside a generic component is not supported — the body becomes a class of its own and `partial class` may carry its type parameters on exactly one part, so that class could not spell {self.type_params.join(", ")}. Move the block into a non-generic component")
            return
        }
        if self.owner == "" {
            self.report(element.span, "<{RENDER_BLOCK_TAG}> needs the class this file is named for, and this emitter was given none")
            return
        }
        var props: List<PropAttr> = []
        for attr: Attr in element.attrs {
            match attr as? PropAttr {
                some(prop) => { props.push(prop) }
                none => {}
            }
        }
        let name: string = "{self.owner}_block{self.block_count}"
        self.block_count = self.block_count + 1

        // The body, emitted by an emitter of its own so its sequence numbers
        // start at 0 — the block is a component, and a component's numbering
        // is its own.
        let inner: Emitter = new Emitter(self.file)
        inner.owner = name
        inner.in_block = true
        let body: Emitted = inner.emit_nodes(element.children)
        for d: Diag in body.diags { self.diags.push(d) }
        for tag: string in body.components { self.note_component(tag) }
        for extra: string in body.blocks { self.blocks.push(extra) }

        var declaration: List<string> = []
        declaration.push("/// Generated from the <{RENDER_BLOCK_TAG}> at {self.file}:{element.span.line}.")
        declaration.push("/// Its props are the only thing that crosses; `props` is this instance.")
        declaration.push("class {name} extends Component \{")
        for prop: PropAttr in props {
            declaration.push("    @param pub {prop.prop_name}: {prop.prop_type} = {default_for(prop.prop_type)}")
        }
        declaration.push("    pub fn init() \{\}")
        declaration.push("    pub override fn render(b: Builder) \{")
        declaration.push("        let props: {name} = self")
        for line: string in body.lines { declaration.push(line) }
        declaration.push("    \}")
        declaration.push("\}")
        self.blocks.push(declaration.join("\n"))

        self.note_component(name)
        let seq: int = self.counters.next()
        self.setup_depth = self.setup_depth + 1
        let c: string = self.setup_name()
        let b: string = self.builder_name()
        self.write(indent, "{b}.component_in<{name}>({seq}, \"{escape_beans_string(mode)}\", fn({c}: {name}) \{{self.trace(element.span)}")
        for prop: PropAttr in props {
            self.check_code(prop.code, prop.span, "a {RENDER_BLOCK_TAG} prop")
            self.write_call(indent + 1, "{c}.{prop.prop_name} = ", prop.code, "")
        }
        self.write(indent, "\})")
        self.setup_depth = self.setup_depth - 1
    }

    /// The body of a `<RenderBlock>`, as the render of a component of its own.
    pub fn emit_nodes(list: List<Node>) -> Emitted {
        self.nodes(list, 2)
        let out: Emitted = new Emitted()
        out.lines = self.lines.clone()
        out.components = self.components.clone()
    ...
    fn emit_parameter(element: ElementNode, attr: Attr, c: string, indent: int) {
        ...
        // `render:mode=` is not a parameter either — `emit_component` read it
        // before this loop and it decides which Builder call to write.
        match attr as? ModeAttr {
            some(_) => { return }
            none => {}
        }
