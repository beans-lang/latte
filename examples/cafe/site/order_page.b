...
//
// `Basket` is hand-written in this block because it has no markup — it is a
// model, not a component, and a `.bx` file compiles the markup of exactly one
// class: the one it is named after.
//
// It is `Basket` and not `Order` because `Order` is a name Beans already has:
// `error: type name 'Order' already taken`. The comparison interface is
// builtin, so a model called `Order` never compiles.
//          
...
partial class OrderPage {
    pub override fn render(b: Builder) {
        b.open(0, "section")  // order_page.bx:77
        b.attr(1, "class", "order")
        b.open(2, "h2")  // order_page.bx:78
        b.text(3, "{self.heading()}")
        b.close()
        b.open(4, "form")  // order_page.bx:80
        b.attr(5, "method", "post")
        b.attr(6, "action", "{self.action()}")
        b.open(7, "input")  // order_page.bx:81
        b.attr(8, "type", "hidden")
        b.attr(9, "name", "{TOKEN_FIELD}")
        b.attr(10, "value", "{self.state.token}")
        b.close()
        b.open(11, "label")  // order_page.bx:83
        b.text(12, "name ")
        b.open(13, "input")  // order_page.bx:84
        b.attr(14, "name", "name")
        b.attr(15, "id", "name")
        b.attr(16, "value", "{self.typed_name()}")
        b.close()
        b.close()
        b.open(17, "label")  // order_page.bx:87
        b.text(18, "cups ")
        b.open(19, "input")  // order_page.bx:88
        b.attr(20, "name", "cups")
        b.attr(21, "id", "cups")
        b.attr(22, "value", "{self.typed_cups()}")
        b.close()
        b.close()
        b.open(23, "label")  // order_page.bx:91
        b.text(24, "decaf ")
        b.open(25, "input")  // order_page.bx:92
        ...
        b.close()
        b.close()
        if b.fold { b.constant(30, "<button type=\"submit\" id=\"place\">place the order</button>") }  // order_page.bx:95
        else {
            b.open(30, "button")  // order_page.bx:95
        ...
        b.close()
        b.open(34, "ul")  // order_page.bx:98
        b.attr(35, "id", "problems")
        for problem: FieldError in self.state.result.errors {  // order_page.bx:99
            b.region(36, "{problem.field}")
            b.open(0, "li")  // order_page.bx:100
            b.text(1, "{problem.field}: {problem.message}")
            b.close()
            b.end_region()
        }
        b.close()
        b.open(37, "p")  // order_page.bx:104
        b.attr(38, "id", "placed")
