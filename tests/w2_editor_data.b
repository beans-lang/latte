...
//
// The suite IS the file. Its golden, `tests/w2_editor_data.out`, is the JSON
// byte for byte, so `editors/shared/bx.json` is a copy of the golden and
...
// scratch file the gate never ran, so nothing said the JSON still described
// the compiler. This one is a suite: `test.sh` runs it on both backends and
// diffs the golden, and `tests/markup.b` §1b asks `html.b`'s predicates about
...
// is a lie the drift gate would otherwise be green about.
package main
