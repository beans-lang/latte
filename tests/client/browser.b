// The browser entry. Eight exports, each one line, forwarding to
// `latte_client`.
//
// They are declared HERE and not in the library because `--emit shared`
// exports the `pub extern "C"` names of the module being BUILT. A generated
// entry is what an application gets; this one is written out so the
// generator has something to be compared against.
package main

import clienttest.site
...
/// One server action's answer. The page calls it when a fetch settles, and
/// the region it names is the one whose batch is now waiting.
...
/// How many calls are waiting for an answer. The development inspector reads
/// it; so does a check that a cancelled call really left.
pub extern "C" fn latte_client_in_flight() -> i32 {
    return actions_in_flight() as i32
}

/// Never called in a browser: `--emit shared` has no `_start`. It is here so
/// the same file also builds as an ordinary program, which is how a
/// check runs the region runtime on a machine with no wasm toolchain.
fn main() {}
